<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="projectList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="单位" align="center" prop="unit" width="150px"/>
      <el-table-column label="价格" align="center" prop="price" width="200px"/>
      <el-table-column label="图片" align="center" prop="image" width="200">
        <template #default="scope">
          <image-preview :src="scope.row.image" :width="50" :height="50"/>
        </template>
      </el-table-column>
      <el-table-column label="护理要求" align="center" prop="nursingRequirement" width="200px"/>
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width" width="230px" fixed="right">
        <template #default="scope">
          ...
          <el-button link type="primary" :icon="scope.row.status == 0 ? 'Unlock' : 'Lock'"
                     ...
                     v-hasPermi="['serve:project:remove']">{{ scope.row.status == 0 ? '启用' : '禁用' }}
          </el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :title="title" v-model="open" width="500px" append-to-body>
      <el-form ref="projectRef" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="排序号" prop="orderNo">
          <el-input-number v-model="form.orderNo" :min="1" :max="20"/>
        </el-form-item>
        <el-form-item label="单位" prop="unit">
          <el-input v-model="form.unit" placeholder="请输入单位"/>
        </el-form-item>
        <el-form-item label="价格" prop="price">
          <el-input-number v-model="form.price" :min="5" :max="200" :step="5"/>
        </el-form-item>
        <el-form-item label="状态" prop="status">
...
function handleEnable(row) {
  ...
  proxy.$modal.confirm(`是否确认${msg}护理项目编号为${_ids}的数据项？`).then(function () {
