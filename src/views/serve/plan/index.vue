          ...
          <el-col :span="24" class="elcolFlex">
            ...
            <el-form-item label="状态：" prop="status">
              <el-radio-group v-model="formData.status" :disabled="isLook">
                <el-radio v-for="{label,value} in nursing_plan_status" :key="value" :label="Number(value)"
              ...
              <div class="tableColumn" v-for="(item, index) in formData.nursingPlanList" :key="index">
                ...
                <div class="column">
                  <el-time-picker v-model="item.executeTime" format="HH:mm:ss" value-format="HH:mm:ss"
                                  :style="{ width: '100%' }" placeholder="请选择时间选择" :disabled="isLook"
                ...
                <div class="column" v-if="!isLook">
                  <el-icon v-if="formData.nursingPlanList.length > 1" @click="handleRowDel(item, index)" class="delect">
                    <Minus/>
                  </el-icon>
...
import {getAllProjects} from '@/api/serve/project';
...
const handleRowDel = (item, index) => {
  if (formData.value.nursingPlanList.length === 1) return;
  formData.value.nursingPlanList.splice(index, 1);
};
...
const getDetails = (id) => {
  getNursingPlan(id).then((response) => {
    ...
    dialogVisible.value = true;
    title.value = '修改护理计划';
    console.log("getDetails", formData.value)
  });
...
function submitForm() {
  //判断选择的护理项目是否有重复的
  if (hasDuplicateIds(formData.value.nursingPlanList)) {
    // 提示重复
